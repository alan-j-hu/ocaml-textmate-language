...
let read_ezjsonm filename =
  ...
  let json =
    Fun.protect
      (fun () -> Ezjsonm.from_channel chan)
      ~finally:(fun () -> close_in chan)
  in
  TmLanguage.of_ezjsonm_exn json

type test_line = { line : string; expected : (int * string list) list }

let check_find grammar scope_name filetypes () =
  let open TmLanguage in
  let t = create () in
  let check p =
    Alcotest.check Alcotest.bool scope_name true
